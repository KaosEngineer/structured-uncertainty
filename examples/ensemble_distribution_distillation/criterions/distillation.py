...
@register_criterion('sequence_reverse_distillation')
class ReverseKLCritertion(FairseqCriterion):
    ...
    def compute_loss(self, model, net_output, ensemble_logits, sample, reduce):
        ...
        log_avg_teacher_probs = torch.log(teacher_probs.mean(2))

        loss = -(probs * log_avg_teacher_probs).sum(-1)
...
@register_criterion('sequence_forward_distillation')
class ForwardKLCritertion(FairseqCriterion):
    ...
    def compute_loss(self, model, net_output, ensemble_logits, sample, reduce):
        ...
        teacher_probs = utils.softmax(ensemble_logits, dim=-1)
        avg_teacher_probs = teacher_probs.mean(2)

        loss = -(avg_teacher_probs * log_probs).sum(-1)
